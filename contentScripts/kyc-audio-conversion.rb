...
themes_to_process = [
  ...
  #{ "name" => "Native History", "prefix" => "cb", "subdirectory" => "caryeBye" }
  #{ "name" => "Native History", "prefix" => "prl", "subdirectory" => "pearlDistrictArtists" }
  ]
...
themes_to_process.each do |theme|
  ...
  Dir.glob("*.{wav,aif}") do |filename|
    ...
    output_file = "#{web_output_directory}/#{new_file_name}.#{web_suffix}"
    ...
    # mp4:
    # `afconvert -v -c 1 -f mp4f -d aac -s 3 -b 128000 \"#{filename}\" #{output_file}`
